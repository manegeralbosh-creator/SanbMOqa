...
# 4. دالات معالجة النصوص وتصفية الأرقام والأسماء من أونكس
...
# 5. خيارات الفترات الزمنية للمتابعة والمطالبة
...
# 6. توزيع شاشات التطبيق عبر التبويبات
tab1, tab2 = st.tabs(["📊 العملاء المستحقين للتذكير اليوم", "📥 رفع وتحديث كشف الحساب الإكسل"])

# 📥 التبويب الثاني: معالجة ورفع كشف أونكس الجديد
with tab2:
...
# 📊 التبويب الأول: فرز ومتابعة المستحقين للمقاضاة اليوم
with tab1:
    ...
    # جلب الحسابات النشطة من المخزن المحلي
    cursor = conn.cursor()
    ...
    if all_customers:
        ...
        # استعراض الحسابات المطلوبة وإجراءات إرسالها واجهياً
        ...
        # استعراض الأرشيف والسجلات التابعة
        st.write("---")
