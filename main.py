...
with tab4:
  ...
  # 2. رفع الملفات (الإكسل + أرشيف الـ ZIP)
  col_ex, col_zip = st.columns(2)
  with col_ex:
    excel_file = st.file_uploader(
        'رفع ملف كشف المبيعات (Excel)', type=['xlsx', 'xls'], key='inv_excel'
    )
  with col_zip:
    zip_file = st.file_uploader(
        'رفع أرشيف الفواتير المضغوط (ZIP)', type=['zip'], key='inv_zip'
    )

  # 3. استخراج وتخزين ملفات الـ PDF من أرشيف الـ ZIP في الذاكرة
  pdf_store = {}
      ...
      with zipfile.ZipFile(zip_file, 'r') as z:
        for file_info in z.infolist():
          if file_info.filename.lower().endswith('.pdf'):
            ...
            # التخزين بعدة صيغ لضمان مطابقة الرقم من الإكسل
            pdf_store[raw_name] = file_bytes
            if clean_num:
              pdf_store[clean_num] = file_bytes

      st.success(f'✅ تم تفكيك وتحميل ملفات الفواتير من الـ ZIP بنجاح!')
    except Exception as e:
      st.error(f'❌ حدث خطأ أثناء قراءة ملف الـ ZIP: {e}')
  ...
  if excel_file is not None:
    try:
      ...
      if not pending_df.empty:
        ...
        # 🖼️ 5. استعراض الفاتورة مباشرة مع خيار التنزيل
        ...
        if pdf_bytes:
          ...
          # معاينة الفاتورة داخل إطار iframe
          ...
          st.warning(
              f'⚠️ لم يتم العثور على ملف PDF مطبق للرقم التسلسلي: (DOCSER_{doc_ser_val}.pdf) داخل ملف الـ ZIP.'
          )
